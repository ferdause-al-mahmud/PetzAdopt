...
const AdoptionRequests = () => {
    ...
    const { mutateAsync: mutateAccept } = useMutation({
        mutationFn: async (adoption) => {
            try {
                const { data } = await axiosSecure.put(`adoption-requests/accept`, adoption);
                return data;
            } catch (error) {
    ...
    const { mutateAsync: mutateDelete } = useMutation({
        mutationFn: async (adoption) => {
            try {
                const { data } = await axiosSecure.delete(`/adoption-request/delete/${adoption._id}`);
                return data;
            } catch (error) {
                throw new Error("An error occurred");
            }
    ...
    return (
        <div>
            <div className="my-8 sm:mt-12">
                ...
                <AdoptionRequestsTable handleAccept={handleAccept} handleReject={handleReject}></AdoptionRequestsTable>
            </div>
        </div>
    );
};
